...
* `web-server` -- принимающий запросы от клиента и обслуживающий статические данные, а так же передающий динамические запросы приложению. На данный момент используется комбинация `nginx` + `uwsgi`, но возможна адаптация под любой способ, поддерживающий `PSGI`.
